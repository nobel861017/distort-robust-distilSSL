...
class UpstreamPretrainExpert(nn.Module):
    ...
    def __init__(
        ...
        self.output_dim = self.upstream_config["distiller"]["final_dim"]
        # self.output_dim = self.model.distiller.final_dim
    ...
    def add_state_to_save(self, all_states):
        ...
        all_states["Up_Config"] = self.upstream_config
        return all_states
    ...
    def get_dev_dataloader(self):
        return self.devloader
    # Interface
    def forward(self, teacher_data, student_data, records={}, global_step=0, log_step=1000, return_other=False, **kwargs):
        """
        Args:
            data:
                [wave_input, pad_mask]
        ...
        teacher_wave_input, teacher_wave_orig, teacher_wave_len, teacher_pad_mask = teacher_data
        teacher_wave_input = teacher_wave_input.to(self.device)
        teacher_wave_len = teacher_wave_len.to(self.device)
        teacher_pad_mask = teacher_pad_mask.type(teacher_wave_input.dtype).to(self.device)

        student_wave_input, student_wave_orig, student_wave_len, student_pad_mask = student_data
        student_wave_input = student_wave_input.to(self.device)
        student_wave_len = student_wave_len.to(self.device)
        student_pad_mask = student_pad_mask.type(student_wave_input.dtype).to(self.device)
        ...
        loss, other_res = self.model(
            ...
            student_wave_len,
            student_pad_mask,
            return_other=(global_step % log_step == 0) or return_other,
        )

        if global_step % log_step == 0 or return_other:
            for key, value in other_res.items():
                if key == "student_features" or key == "teacher_features":
                    records[key] = value
...
class DistillerForPretrain(nn.Module):
    ...
    def __init__(self, config: DistillerConfig, teacher_config: edict):
        ...
        self.encoder_layers = self.distiller.config.encoder_layers
    ...
    def forward(
        self,
        teacher_wave_input: torch.Tensor,
        teacher_wave_orig: list,
        teacher_wave_len: torch.Tensor,
        teacher_pad_mask: torch.Tensor,
        student_wave_input: torch.Tensor,
        student_wave_orig: list,
        student_wave_len: torch.Tensor,
        student_pad_mask: torch.Tensor,
        ...
        # Forward model
        student_feat, student_feat_final, student_pred, student_pad_mask, student_hiddens = self.distiller(student_wave_input, student_pad_mask, get_hidden=True)
        
        with torch.no_grad():
            teacher_wave_orig = [wave.to(teacher_wave_input.device) for wave in teacher_wave_orig]
            with torch.cuda.amp.autocast(False):
                teacher_hiddens = self.teacher(teacher_wave_orig)
            self.teacher_feature_dict = teacher_hiddens
            # len(self.teacher_feature_dict["hidden_states"]) = 13
        ...
        (
            ...
            sim_loss,
            sim_layer_loss,
        ) = self.compute_loss(student_feat, student_pred, teacher_hiddens, return_other)
        ...
        if return_other:
            with torch.no_grad():
                other_res = {
                    ...
                    "feat_pen": feat_pen,
                    "sim_loss": sim_loss,
                    "norm_feat_final": student_feat_final.pow(2).mean(),
                    "student_features": {"hidden_states": student_hiddens, "last_hidden_state": student_hiddens[-1].unsqueeze(dim=0), "pred": student_pred.permute(1, 0, 2, 3)},
                    "teacher_features": {"hidden_states": self.teacher_feature_dict["hidden_states"][1:self.encoder_layers+1], "last_hidden_state": self.teacher_feature_dict["hidden_states"][self.encoder_layers].unsqueeze(dim=0), "pred": teacher_hiddens.permute(1, 0, 2, 3)}
                }
